...
# Veri setini yükleme işlemi yapacağım
...
# Eksik değerleri kontrol edip, ortalama ile dolduruyorum
if df.isnull().sum().sum() > 0:
    df = df.fillna(df.mean(numeric_only=True))

# İl bilgisini ayırıp veriyi ölçekliyorum
iller = df["İl Province"]
X = df.drop(columns=["İl Province"])
X_scaled = MinMaxScaler().fit_transform(X)

# KMeans ile kümeleme işlemi 
k = 4
kmeans = KMeans(n_clusters=k, random_state=42, n_init="auto")
kmeans_labels = kmeans.fit_predict(X_scaled)

# DBSCAN ve Agglomerative ile alternatif kümelemeler
...
# Etiketleri ana veri çerçevesine ekliyorum
...
plt.ylabel("Bölgesel Özellik Skoru")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()

# Random Forest ile öznitelik önem dereceleri işlemi yapacağım
...
# Benzer il öneri sistemi oluşturmak için kosinüs benzerliğinden yararlanacağım
